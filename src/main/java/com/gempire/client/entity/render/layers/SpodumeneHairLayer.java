...
public class SpodumeneHairLayer<E extends EntityGem, M extends ModelGem<E>> extends GempireLayer<EntityGem, ModelGem<EntityGem>> {
    ...
    public SpodumeneHairLayer(IEntityRenderer<EntityGem, ModelGem<EntityGem>> entityRendererIn) {
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, EntityGem gem, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        IVertexBuilder builder = bufferIn.getBuffer(RenderType.getEntityTranslucent(new ResourceLocation(gem.getModID()+":textures/entity/spodumene/hair_" + gem.getHairVariant() + ".png")));
        this.getEntityModel().setRotationAngles(gem, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch);
        this.getEntityModel().render(matrixStackIn, builder, packedLightIn, OverlayTexture.NO_OVERLAY, r, g, b, 1F);
